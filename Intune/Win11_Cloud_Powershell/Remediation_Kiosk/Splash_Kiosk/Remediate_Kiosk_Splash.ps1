...
$imagePathLogo  = Join-Path $folderPath "LogoCwcis.png"
...
<img src="C:\TG\Splash_Kiosk\LogoCwcis.png" alt="Gwynedd Logo">
...
# Download logo image if missing
if (-not (Test-Path $imagePathLogo)) {
    try {
        Invoke-WebRequest "https://www.gwynedd.llyw.cymru/SiteElements/Images/LogoCwcis.png" `
            -OutFile $imagePathLogo `
            -UseDefaultCredentials
    }
    catch {
        Write-Output "Image download failed"
