...
# Package Type : Intune Win32 App (Self-contained offline 'files' bundle)
...
# These Files will be copied locally from the packaged 'files' directory
$files = @(
    # Copy Cyngor Gwynedd background to local source
    @{source = "$PSScriptRoot\files\Cefndir.png"; output = "C:\Windows\Wallpaper\Cefndir.png"},
    @{source = "$PSScriptRoot\files\CefndirLock.png"; output = "C:\Windows\Web\Screen\CefndirLock.png"},

    # Script to set initial settings on First Login
    @{source = "$PSScriptRoot\files\Initialize_User.ps1"; output = "C:\Windows\Initialization\Initialize_User.ps1"},
    
    # Copy Gwynedd Icons to local directory
    @{source = "$PSScriptRoot\files\Gwynedd.ico"; output = "C:\TG\Eiconau\Gwynedd.ico"},
    @{source = "$PSScriptRoot\files\logo_cyngor_gwynedd.ico"; output = "C:\TG\Eiconau\logo_cyngor_gwynedd.ico"},
        
    # Copy Admin Unlock File Locally
    @{source = "$PSScriptRoot\files\AdministrativeUnlock.dll"; output = "C:\Windows\System32\AdministrativeUnlock.dll"}
)
...
$Regs = @(
    ...
    # Set Default User Settings for all users (Mounted Hive HKLM:\DEFUSER)
...
# Copy the local files out of the deployment package into their target positions
foreach ($file in $files) {
    ...
    if (Test-Path -Path $file.source) {
        Write-Output "Copying file from package: $($file.source) -> $($file.output)"
        try {
            Copy-Item -Path $file.source -Destination $file.output -Force -ErrorAction Stop
        } catch {
            Write-Warning "Failed to copy file $($file.source). Error: $_"
        }
    } else {
        Write-Warning "Missing source file inside package: $($file.source)"
    }
}
...
Write-Output "Mounting Default User Profile Hive..."
...
try {
    ...
    # This always executes, safeguarding the default hive against getting permanently locked down if mistakes occur above
...
# Remote Help Shortcut i bob defnyddiwr
$RemoteHelpExe = "C:\Program Files\Remote Help\RemoteHelp.exe"

if (Test-Path $RemoteHelpExe) {
    $RemoteHelpShortcut = $Shell.CreateShortcut("$PublicDesktop\Remote Help.lnk")
    $RemoteHelpShortcut.TargetPath = $RemoteHelpExe
    $RemoteHelpShortcut.IconLocation = "$RemoteHelpExe,0"
    $RemoteHelpShortcut.Save()

    Write-Output "Remote Help shortcut created."
}
else {
    Write-Warning "Remote Help executable not found at: $RemoteHelpExe"
}
